...
class Moderation(commands.Cog):
    ...
    async def on_ready(self):
        ...
        await self.bot.primedb.execute(
            "create table if not exists userstrikes (caseid serial primary key, guildid bigint, userid bigint, reason varchar(255), dateissued timestamp, modresp bigint)"
        )
        print("mod table plugged")

    async def sendtoauditlog(
        self,
        command,
        user: discord.Member,
        ...
        if command in Actionactions:
            em = discord.Embed(
                title="Mod Action",
                description=f"{command} used by {user} in {channel} with reason {reason}",
            ...
            if command == "Unmute":
                ...
                em.add_field(
                    name="Action", value=f"{victim} was unmuted automatically."
                )
                em.add_field(name="Action", value=f" Reason : {reason}")
                em.set_thumbnail(url=victim.avatar_url)
    ...
    async def trackuser(self, ctx, user: discord.Member = None):
        ...
        if user is None:
            await ctx.send(
                f"{ctx.author.mention}, please mention a valid user to track them."
            )

        em = discord.Embed(colour=discord.Colour.blue(), description="Track")
        result = await self.bot.primedb.fetchrow(
            "SELECT * FROM userstrikes WHERE guildid = $1 and userid = $2",
            int(ctx.guild.id),
            int(user.id),
        )
        ...
        if result is None:
            ...
            for i in range(0, len(data)):
                em.add_field(
                    name=f"Case Id : {data[i][0]}",
                    value=f"```Reason :{data[i][3]}``` \n```Time Issued : {data[i][4]}``` \n**Responsible Moderator** : <@{data[i][5]}> ",
                    inline=True,
                )
    ...
    async def strike(self, ctx, user: discord.Member = None, *, reason: str = None):
        ...
        em.add_field(
            name="Action", value=f"{ctx.author.mention} gave a strike to {user}."
        )
        em.add_field(name="Action", value=f" Reason : {reason}")
        ...
        if user is None:
            ...
            if reason is None:
                await ctx.send(
                    f"{ctx.author.mention}, please specify the reason and strike again."
                )
            else:
                await self.bot.primedb.execute(
                    f"insert into userstrikes (guildid, userid,reason,dateissued,modresp) values ($1,$2,$3,$4,$5)",
                    ctx.guild.id,
                    int(user.id),
                    reason,
                ...
                await ctx.send(embed=em)
                await user.send(
                    f"You were given a strike in Primordia by {ctx.author.mention} for the reason : \n ```{reason}``` \n on {ctx.message.created_at}. If you would like to appeal this strike, fill out this form :  https://forms.gle/Z8z53Ec78zqW6BLH8."
                )
                await self.sendtoauditlog(
                    "Strike", ctx.author, ctx.channel, user, reason
                )
    ...
    @commands.has_guild_permissions(mute_members=True)
    async def mute(
        self, ctx, member: discord.Member, time: int, d, *, reason: str = None
    ):
        ...
        if reason is None:
            ...
            await member.send(
                f"If you would like to appeal this decision, fill out this form : https://forms.gle/Z8z53Ec78zqW6BLH8"
            )
    ...
    async def dump(self, ctx, member: discord.Member):
        ...
        emb = discord.Embed(
            description=f"{member.mention} has been banished.",
    ...
    async def removexp(self, ctx, member: discord.Member = None, amount: int = 0):
        ...
        await self.bot.primedb.execute(
            "update userlevels set xp = xp - $1 where guildid = $2 AND userid = $3",
            amount,
            ctx.guild.id,
            member.id,
        )
        ...
        await self.sendtoauditlog(
            "XP Remove", ctx.author, ctx.channel, member, "XP Removed"
        )
    ...
    async def removeferins(self, ctx, member: discord.Member = None, amount: int = 0):
        ...
        await self.sendtoauditlog(
            "Ferin Remove", ctx.author, ctx.channel, member, "Ferin Removed"
        )
    ...
    async def addferins(self, ctx, member: discord.Member = None, amount: int = 0):
        ...
        await self.sendtoauditlog(
            "Ferin Add", ctx.author, ctx.channel, member, "Ferin Added"
        )
