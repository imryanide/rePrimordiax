...
class general(commands.Cog):
    ...
    async def on_ready(self):
        ...
        await self.bot.primedb.execute("create table if not exists weatherevents (weather text, description text, url text)")
    ...
    async def createweather(self,ctx):
        ...
        await ctx.send(f"{ctx.author.mention}, what will the name of the weather event be?")
        try:
            wthr = await self.bot.wait_for('message',check = is_correct, timeout = 15.0)
            if wthr.content in allevs:
                await ctx.send(f'{ctx.author.mention}, that weather already exists. Try again.')
                return
        ...
        try:
            ...
            if not isurl(wurl.content):
                await ctx.send(f'{ctx.author.mention}, this is not a valid image url. Please try again.')
                return
        ...
        if wthr and dscp and wurl:
            report = await self.bot.primedb.fetch("INSERT INTO weatherevents (weather, description, url) VALUES ($1, $2, $3) RETURNING *",wthr.content, dscp.content, wurl.content)
    ...
    async def weather(self,ctx):
        report = await self.bot.primedb.fetch("SELECT * FROM weatherevents ORDER BY RANDOM() LIMIT 1")
        em=discord.Embed(title = "Weather Report",color = discord.Color.purple(),description = f"```{report[0]['description']}```")
        em.set_image(url = report[0]['url'])
        await ctx.send(embed=em)
    ...
    async def help(self,ctx):
        em=get_main_help_embed(description="Use `p!help` Command for information.", picture=self.bot.user.avatar_url)

        em.add_field(name="Moderation", value = f"``` kick \n ban \n strike \n mute \n unmute \n trackuser \n dump```")
        em.add_field(name="Ferin", value=f"``` balance \n fleaderboard \n work \n daily \n gamble ```")
    ...
    async def kick(self, ctx):
        ...
        em = get_help_embed(title="Kick", description= "Kicks a member from the server.",picture=self.bot.user.avatar_url)
    ...
    async def ban(self, ctx):
        ...
        em = get_help_embed(title="Ban", description= "Bans a member from the server.",picture=self.bot.user.avatar_url)
    ...
    async def strike(self, ctx):
        ...
        em = get_help_embed(title="Strike", description= "Adds a strike to a user.",picture=self.bot.user.avatar_url)
        em.add_field(name="**Syntax**", value=f"`p!strike <member> [reason]` \n Using `!xp take <member>` will also add a strike to the user.")
        await ctx.send(embed=em)

    @help.command()
    async def mute(self,ctx):
        em = get_help_embed(title="Mute", description= "Mutes and adds a strike to a user.",picture=self.bot.user.avatar_url)
    ...
    async def trackuser(self,ctx):
        em = get_help_embed(title="Track", description= "Tracks a user.",picture=self.bot.user.avatar_url)
    ...
    async def leaderboard(self, ctx):
        ...
        em = get_help_embed(title="Leaderboard", description= "Displays leaderboard of people with most xp.",picture=self.bot.user.avatar_url)
    ...
    async def rank(self, ctx):
        ...
        em = get_help_embed(title="Rank", description= "Displays rank.",picture=self.bot.user.avatar_url)
