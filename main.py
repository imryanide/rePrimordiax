...
async def create_db_pool():
    bot.primedb = await asyncpg.create_pool(
        host="135.181.44.255",
        user="primeuser",
        password="Fantomex11",
        database="PrimeDB",
...
async def called_once_a_day():
    ...
    if date.today().weekday() == 0:
        ...
        report = await bot.primedb.fetch(
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.errors.CommandNotFound):
        await ctx.send(
            f"The command you specified was not found. Type p!help to see all available commands."
        )
        ...
        await ctx.send(
            "You don't have the required permissions for that command. Contact Staff if you think that is a mistake."
        )

    elif isinstance(error, commands.errors.CommandOnCooldown):
        await ctx.send(
            f"You need to wait {error.retry_after:,.2f} seconds before trying this command again."
        )
